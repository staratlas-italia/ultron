...
export const scanSdu = async (fleet: SageFleet, counter?: number) => {
  // action starts
  ...
  const sdt = await fleet
    .getSageGame()
    .buildAndSendDynamicTransactions(ix.ixs, false, 1);
  if (sdt.type !== "Success") throw new Error(sdt.type); // retry entire action

  // other
  console.log(`Scan completed!`);
  console.log(
    `\nWaiting Scan Cooldown for ${
      fleet.getStats().miscStats.scanCoolDown
    } seconds...`
  );
