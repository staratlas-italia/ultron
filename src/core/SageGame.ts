...
export class SageGame {
  ...
  private customPriorityFee: PriorityFee = {
    type: "none",
  };
  ...
  static async initFrom({
    ...
    keypair,
    priorityFee,
  }: Pick<Conf, "connection" | "keypair" | "priorityFee">): Promise<SageGame> {
    const game = new SageGame(keypair, connection, priorityFee);
  ...
  private async buildDynamicTransactions(instructions: InstructionReturn[]) {
    const getFee = async (
      ...
      if (this.customPriorityFee.type === "custom") {
        return 0;
      }

      const customPriorityFee = priorityLevelValue[this.customPriorityFee.type];
